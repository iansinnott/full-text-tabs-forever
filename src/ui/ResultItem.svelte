<script lang="ts">
  ...
  import { getFaviconByUrl, cleanUrl, getRelativeTime, getFullLocalTime } from "@/common/utils";
...
>
  ...
  {#if showSnippets && item.snippet && item.attribute !== "title" && item.attribute !== "url"}
    <div data-has-snippet="1" class="pl-7 truncate">
      <!-- Using HTML is to allow the <mark> tags generated by the DB. This should be sanitized -->
      {@html item.snippet}
    </div>
  {/if}
